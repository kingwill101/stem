...
tweak headers/meta/queue at call sites? Wrap the definition in a
`TaskEnqueueBuilder` and invoke `await builder.enqueue(enqueuer);` or
`await builder.enqueueAndWait(caller);`.
