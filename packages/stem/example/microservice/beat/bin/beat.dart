...
Future<void> _seedSchedules(ScheduleStore store, String path) async {
  ...
  for (final element in entries) {
    ...
    if (_scheduleConfigsEqual(existing, entry)) {
      stdout.writeln(
        'Schedule ${entry.id} already matches seed configuration; skipping.',
      );
      continue;
    }

    final updated = entry.copyWith(version: existing.version);
    stdout.writeln(
      'Updating schedule ${entry.id}: existing version ${existing.version}, '
      'payload version ${updated.version}',
    );
    await store.upsert(updated);
    stdout.writeln(
      'Updated schedule ${entry.id} -> ${entry.taskName} (${_describeSpec(entry.spec)})',
    );
  }
}
...
bool _scheduleConfigsEqual(ScheduleEntry a, ScheduleEntry b) {
  if (a.taskName != b.taskName) return false;
  if (a.queue != b.queue) return false;
  if (a.enabled != b.enabled) return false;
  if (a.jitter != b.jitter) return false;
  if (a.timezone != b.timezone) return false;
  if (!_deepEquals.equals(a.spec.toJson(), b.spec.toJson())) return false;
  if (!_deepEquals.equals(a.args, b.args)) return false;
  if (!_deepEquals.equals(a.kwargs, b.kwargs)) return false;
  if (!_deepEquals.equals(a.meta, b.meta)) return false;
  return true;
}
