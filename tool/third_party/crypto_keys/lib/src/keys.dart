...
class KeyPair {
  ...
  factory KeyPair.generateRsa({BigInt? exponent, int bitStrength = 2048}) {
    ...
    if (publicKey is! pc.RSAPublicKey || privateKey is! pc.RSAPrivateKey) {
      throw StateError('Unexpected RSA key types from Pointy Castle.');
    }
  ...
  factory KeyPair.generateEc(Identifier curve) {
    ...
    if (publicKey is! pc.ECPublicKey || privateKey is! pc.ECPrivateKey) {
      throw StateError('Unexpected EC key types from Pointy Castle.');
    }
    ...
    return KeyPair(
        publicKey: EcPublicKey(
            ...
            curve: curve),
        privateKey:
            EcPrivateKey(eccPrivateKey: privateKey.d!, curve: curve));
  }
