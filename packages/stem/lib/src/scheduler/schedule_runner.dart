...
/// One-shot schedule execution and task dispatch.
///
/// This library provides the [ScheduleRunner] class, which reads the
/// [ScheduleStore], ensuring task uniqueness via [LockStore], and publishing
/// due tasks to the [Broker].
///
/// ## Distributed Scheduling
///
/// When a [LockStore] is provided, runners coordinate dispatch using locks.
/// Publication and schedule persistence are not transactional: consumers must
/// remain idempotent when a crash occurs between these operations.
library;
...
///
/// Each pass checks tasks that have reached their `nextRunAt` timestamp,
/// coordinating distributed locks, signing, and metric recording.
class ScheduleRunner {
  /// Creates a scheduler instance.
  ///
  /// - [store]: Persistence for schedule definitions and execution state.
  /// - [publisher]: The transport layer for dispatching tasks.
  ...
  /// Runs one scheduling pass. Temporary lock-renewal timers, if needed,
  /// are cancelled before this future completes.
